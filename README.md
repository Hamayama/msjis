...
- msjis-mode の引数  
  ...
  - 第3引数の use-api には、コンソールの入出力に  
    ...
    Windows API を使用すると、Unicode の文字が使用可能になるケースがあります。  
    (詳細は注意事項の 4. を参照ください)  
    以下の値を指定可能です。省略時は #f を指定したことになります。
    
    ```
    #f : Windows API を使用しない(デフォルト)
...
4. (以下の記述は、Windows 8.1 (64bit), Gauche v0.9.5_pre1 でのみ動作確認しています)  
   コンソールエミュレータの ConEmu ( https://conemu.github.io/ )  
   ...
   (備考) コードページ CP932 のままでも、Unicode の文字を表示できるようです。  
   (仕様なのかバグなのか分からないが。。。)  
   この場合、行の折り返しは一見正常になりますが、サロゲートペアの文字があると、  
   1文字を2文字分と判定して折り返してしまいます。  
   
   (備考) 絵文字 (#\u1F363 の「寿司」等) については、表示はされますがカーソル位置が  
