...
- msjis-mode の引数  
  ...
  - 第2引数の ces には、変換する文字エンコーディングを指定します。  
    ...
    (げた記号等に変換されます)  
    ただし 文字エンコーディングのSJISとCP932では、登録されている文字が一部異なるため、  
    入力の内容によっては、結果が変わってしまうことがあります。
  ...
  - 第3引数の userwc には、リダイレクトなしの場合に、  
    ...
    ```
    本設定は、コマンドプロンプトでコードページ CP65001(UTF-8) を選択する場合には、  
    `#t` にする必要があります。  
    (CP65001(UTF-8) 選択中は、ReadConsole(),WriteConsole() を使用しないと、  
     表示に不具合が発生します)  
    ただし 本設定は、Gauche v0.9.4 以降でないと正常に動作しません。  
    また 現状の Windows には、「コマンドプロンプトで CP65001(UTF-8) を選択すると、  
    日本語フォントやMS-IMEが使用できなくなる」 という大きな制約があります。
