...
- msjis-mode の引数  
  ...
  - 第3引数の c65001 には、リダイレクトなしの場合に、  
    ...
    ```
    #t : CP65001(UTF-8) に対応する処理を行う(※)
    #f : CP65001(UTF-8) に対応する処理を行わない(デフォルト)
    
    (※) 本設定は、Gauche v0.9.4 以降でないと正常に動作しません。
         また、Windows XP では入力で文字化けが発生することがあります。
    ```
...
4. コンソールエミュレータの ConEmu ( https://code.google.com/p/conemu-maximus5/ )  
   ...
   ことができません。これは、現状の制限事項になります。  
   (原因は Win32 API の WriteConsole() が 文字を UCS-2 として処理しているためのようです。  
    WriteConsole() を使うのをやめて、WriteFile() を使い、さらに、WriteFile() の  
    「書き込めたバイト数」を無視するようにすれば、表示することはできるようです。  
    (WriteFile() の「書き込めたバイト数」が実際と異なることがゴミ付加の原因です)  
    しかし、表示まではできても、カーソル移動や文字削除が2文字分の扱いになるため、  
    結局、編集でおかしくなります。  
    現状では、サロゲートペアの文字は使えないと考えたほうがよいようです)
