...
4. (以下の記述は、Windows 8.1 (64bit), Gauche v0.9.4 でのみ動作確認しています)  
   ...
   ```
     ...
     (print #\u03B1#\u03B2#\u03B3#\u03BB)
   ```
   ただし、サロゲートペアの文字 (#\u2000B 等) については、うまく表示することが  
   できません。これについては、制限事項になります。  
   (Gauche の開発最新版では、サロゲートペアの文字も表示可能です)  
   
   (注意) ConEmu 上で、日本語のフォントが重なって表示される場合には、  
   ConEmu の Settings メニュー で Main を選択し、その中の Monospace の設定を  
   確認してください。  
   Monospace のチェックを外すと、とりあえずフォントの重なりは解消されます。  
   ただしこのとき、コードページ CP65001(UTF-8) では、全角文字を含んだ行の表示が  
   画面に入りきらないケースが発生します。  
   一方、Monospace にチェックを入れて、さらに Cell 幅を設定すると、行の折り返しは  
   正常になります。ただしこの場合は、全角文字と半角文字が同じ幅で表示されます。
