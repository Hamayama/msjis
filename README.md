...
4. (以下の記述は、Windows 8.1 (64bit), Gauche v0.9.5_pre1 でのみ動作確認しています)  
   ...
   しかし、コンソールエミュレータの ConEmu ( https://conemu.github.io/ ) 等では、  
   コードページ CP65001(UTF-8) を選択して、おおむね問題なく動作させることができます。  
   この環境で 本モジュールを使う場合には、msjis-mode の第3引数に #t を指定してください。  
   ...
   ```
     ...
     (print #\u03B1#\u03B2#\u03B3#\u03BB)
   ```
   (備考) ConEmu 上で、日本語のフォントが重なって表示される場合には、  
   ...
   行の折り返しは正常になります。ただしこの場合には、全角文字と半角文字が同じ幅で  
   表示されます (見づらい) 。  
