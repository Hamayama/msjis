...
- msjis-mode の引数  
  ...
  - 第2引数の ces には、変換する文字エンコーディングを指定します。  
    ...
    ```
    #f     : 変換する文字エンコーディングを自動設定する(デフォルト(※1))
    'CP932 : 変換する文字エンコーディングをCP932とする(※2)
    'SJIS  : 変換する文字エンコーディングをSJISとする(※3)
    'UTF-8 : 変換する文字エンコーディングをUTF-8とする(※4)
    
    (※1) コンソールのコードページの情報を取得して、cesを自動設定します。
    (※2) 日本語版Windowsのデフォルトです。
    (※3) 文字エンコーディングのCP932とSJISとでは、使用可能な文字が一部異なります。
...
4. (以下の記述は、Windows 8.1 (64bit), Gauche v0.9.5_pre1 でのみ動作確認しています)  
   コンソールエミュレータの ConEmu ( https://conemu.github.io/ ) 等では、  
   コードページ CP65001(UTF-8) を選択して使用することができます。  
   これによって、ユニコードの文字が使用可能になります。  
   ...
   ```
     ...
     (print #\u03B1#\u03B2#\u03B3#\u03BB)
   ```
   (注意) ConEmu 上で、日本語のフォントが重なって表示される場合には、  
   ConEmu の Settings メニュー で Main を選択し、その中の Monospace の設定を確認してください。  
   Monospace のチェックを外すと、とりあえずフォントの重なりは解消されます。  
   ただしこのとき、コードページ CP65001(UTF-8) では、全角文字を含んだ行の表示が  
   画面に入りきらないケースが発生します。  
   一方、Monospace にチェックを入れて、さらに Cell 幅を 16 等に設定すると、  
   行の折り返しは正常になります。ただしこの場合は、全角文字と半角文字が同じ幅で  
   表示されます (見づらい) 。  
   
   (注意) サロゲートペアの文字 (#\u20B9F の「しかる」等) については、  
   Gauche v0.9.4 以前では表示できません。Gauche の開発最新版では表示できます。  
   ただし、カーソル移動で2文字分移動しないと前後の文字に移動できません。  
   また、サロゲートペアの文字については、画面の右端での折り返しが正常に行われない  
   ケースがあるようです(文字が消えたり化けたりします)。  
   
   (注意) コードページ CP932 のままでも、ユニコードの文字を表示できるようです。  
   (仕様なのかバグなのか分からないが。。。)  
   この場合、行の折り返しは一見正常になりますが、サロゲートペアの文字があると、  
   1文字を2文字分とカウントして折り返してしまいます。  
   
   (注意) 絵文字 (#\u1F363 の「すし」等) については、表示はされますがカーソル位置が  
   ずれます。上記設定で Monospace にチェックを入れて Cell 幅を 16 等に設定すれば、  
   ずれなくできます (ただし見づらい) 。  
   
   (注意) 現状、コマンドプロンプトのみでは、コードページ CP65001(UTF-8) は、正常に動作しません。  
   (日本語フォントが選択できない。MS-IMEが起動しない等の問題があります)  
   したがって、ConEmu 等のソフトウェア上での実行が必要になります。
