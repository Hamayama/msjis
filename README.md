...
- msjis-mode の引数  
  ...
  - 第3引数の c65001 には、リダイレクトなしの場合に、  
    ...
    (※2) 具体的には、Win32 API の ReadConsole(),WriteConsole() を使用します。
          これらは、Gauche v0.9.4 以降でないと正常に動作しません。
          また、Windows XP で ReadConsole() を使用すると、
          文字化けが発生することがあります。
          また、コマンドプロンプトで CP65001(UTF-8) を使おうとすると、
          現状の Windows には、いろいろと制限事項があります。
...
4. (以下の記述は、Windows 8 (64bit), Gauche v0.9.4 でのみ動作確認しています)  
