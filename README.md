...
4. コマンドプロンプトでコードページ CP65001(UTF-8) を選択して使用する場合には、  
   msjis-mode の第3引数に #t を指定する必要があります。  
   これは、CP65001(UTF-8) 選択時には、ReadConsole(),WriteConsole() を使用しないと、  
   文字の後ろにゴミが付加される等の不具合が発生するためです。  
   (ただし 現状の Windows には、「コマンドプロンプトで CP65001(UTF-8) を選択すると、  
    日本語フォントやMS-IMEが使えなくなる」 という制限があるため、  
    コードページ CP65001(UTF-8) は使いづらいものになっています)
